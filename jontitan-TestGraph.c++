...
To test the program:
    ...
    %
    % g++ -ansi -pedantic -I ~/program/boost_1_50_0 -lcppunit -ldl -Wall TestGraph.c++ -o TestGraph.app
*/
...
struct TestGraph : CppUnit::TestFixture {
    ...
    edge_descriptor edAI;
    edge_descriptor edLK;
    ...
    void test_add_edge_4 () {
        std::pair<edge_descriptor, bool> p = add_edge(vdA, vdI, g);
        CPPUNIT_ASSERT(p.second == true);
    ...
    void test_adjacent_vertices_2() {
        std::pair<adjacency_iterator, adjacency_iterator> p = adjacent_vertices(vdJ, g);
        adjacency_iterator b = p.first;
        adjacency_iterator e = p.second;
        CPPUNIT_ASSERT(b == e);
        add_edge(vdJ, vdA, g);
        p = adjacent_vertices(vdJ, g);
        b = p.first;
        e = p.second;
        CPPUNIT_ASSERT(b != e);
        vertex_descriptor vd = *b;
    ...
    void test_edge_2 () {
        std::pair<edge_descriptor, bool> p = edge(vdA, vdI, g);
        CPPUNIT_ASSERT(p.second == false);}
        
    void test_edge_3 () {
        edAI = add_edge(vdA, vdI, g).first;
        std::pair<edge_descriptor, bool> p = edge(vdA, vdI, g);
        CPPUNIT_ASSERT(p.first  == edAI);
        CPPUNIT_ASSERT(p.second == true);
        p = edge(vdI, vdA, g);
        CPPUNIT_ASSERT(p.second == true);}
    ...
    void test_edges_3 () {
        vdK = add_vertex(g1);
        vdL = add_vertex(g1);
        edKL = add_edge(vdK, vdL, g1).first;
        edges_size_type es = num_edges(g1);
        CPPUNIT_ASSERT(es == 1);
        edLK = add_edge(vdL, vdK, g1).first;
        es = num_edges(g1);
        CPPUNIT_ASSERT(es == 2);
        std::pair<edge_iterator, edge_iterator> p = edges(g1);
        edge_iterator                           b = p.first;
        ...
        CPPUNIT_ASSERT(b != e);
        edge_descriptor ed = *b;
        CPPUNIT_ASSERT(ed == edKL);
        ++b;
        ed = *b;
        CPPUNIT_ASSERT(ed == edLK);
        ++b;
        CPPUNIT_ASSERT(b == e);}
    ...
    void test_source_2 () {
        vertex_descriptor vd = source(edAI, g);
        CPPUNIT_ASSERT(vd == vdA);}
        
    void test_source_3 () {
        vertex_descriptor vd = source(add_edge(vdB, vdA, g).first , g);
        CPPUNIT_ASSERT(vd == vdB);}
    ...
    void test_target_2 () {
        vertex_descriptor vd = target(edAI, g);
        CPPUNIT_ASSERT(vd == vdA);}
    
    void test_target_3  () {
        vertex_descriptor vd = target(add_edge(vdB, vdA, g).first, g);
        CPPUNIT_ASSERT(vd == vdA);}
	...
	void test_topological_sort_2 () {
        ...
        vdL = add_vertex(g1);
        edLK = add_edge(vdL, vdK, g1).first;
        edLK = add_edge(vdK, vdL, g1).first;
        std::ostringstream out;
        topological_sort(g1, std::ostream_iterator<vertex_descriptor>(out, " "));
