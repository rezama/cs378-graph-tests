// ----------------------------
// projects/graph/TestGraph.c++
// Copyright (C) 2012
// Glenn P. Downing
// ----------------------------

/*
To test the program:
    % ls /usr/include/cppunit/
    ...
    HelperMacros.h
    ...
    % locate libcppunit.a
    /usr/lib/libcppunit.a
    % ls /public/linux/include/boost-1_44/boost/graph/
    ...
    adjacency_list.hpp
    ...
    % g++ -ansi -pedantic -I/public/linux/include/boost-1_44 -lcppunit -ldl -Wall TestGraph.c++ -o TestGraph.app
    % valgrind TestGraph.app >& TestGraph.c++.out
*/

// --------
// includes
// --------

#include <iostream> // cout, endl
#include <iterator> // ostream_iterator
#include <sstream>  // ostringstream
#include <utility>  // pair

#include "boost/graph/adjacency_list.hpp" // adjacency_list
#include "boost/graph/exception.hpp"

#include "cppunit/extensions/HelperMacros.h" // CPPUNIT_TEST, CPPUNIT_TEST_SUITE, CPPUNIT_TEST_SUITE_END
#include "cppunit/TestFixture.h"             // TestFixture
#include "cppunit/TestSuite.h"               // TestSuite
#include "cppunit/TextTestRunner.h"          // TestRunner

#include "Graph.h"

// ---------
// TestGraph
// ---------

template <typename T>
struct TestGraph : CppUnit::TestFixture {
    // --------
    // typedefs
    // --------

    typedef T                                       graph_type;

    typedef typename graph_type::vertex_descriptor  vertex_descriptor;
    typedef typename graph_type::edge_descriptor    edge_descriptor;

    typedef typename graph_type::vertex_iterator    vertex_iterator;
    typedef typename graph_type::edge_iterator      edge_iterator;
    typedef typename graph_type::adjacency_iterator adjacency_iterator;

    typedef typename graph_type::vertices_size_type vertices_size_type;
    typedef typename graph_type::edges_size_type    edges_size_type;

    // -----
    // tests
    // -----

    // directed, sparse, unweighted
    // possibly connected
    // possibly cyclic
	graph_type g;

    vertex_descriptor vdA;
    vertex_descriptor vdB;
    vertex_descriptor vdC;
    vertex_descriptor vdD;
    vertex_descriptor vdE;
    vertex_descriptor vdF;
    vertex_descriptor vdG;
    vertex_descriptor vdH;

    edge_descriptor edAB;
    edge_descriptor edAC;
    edge_descriptor edAE;
    edge_descriptor edBD;
    edge_descriptor edBE;
    edge_descriptor edCD;
    edge_descriptor edDE;
    edge_descriptor edDF;
    edge_descriptor edFH;
    edge_descriptor edGH;
	edge_descriptor edFD; // creates a cycle

    // -----
    // setUp
    // -----

    void setUp () {
		vdA  = add_vertex(g);
        vdB  = add_vertex(g);
        vdC  = add_vertex(g);
        vdD  = add_vertex(g);
		vdE  = add_vertex(g);
		vdF  = add_vertex(g);
		vdG  = add_vertex(g);
		vdH  = add_vertex(g);
        edAB = add_edge(vdA, vdB, g).first;
        edAC = add_edge(vdA, vdC, g).first;
        edAE = add_edge(vdA, vdE, g).first;
        edBD = add_edge(vdB, vdD, g).first;
        edBE = add_edge(vdB, vdE, g).first;
        edCD = add_edge(vdC, vdD, g).first;
        edDE = add_edge(vdD, vdE, g).first;
        edDF = add_edge(vdD, vdF, g).first;
        edFH = add_edge(vdF, vdH, g).first;
        edGH = add_edge(vdG, vdH, g).first;}
		
	// ---------------
    // test_add_vertex
    // ---------------

	void test_add_vertex_1 () {
		graph_type g0;
	    vertices_size_type s = num_vertices(g0);
		CPPUNIT_ASSERT(s == 0);
        vertex_descriptor v = add_vertex(g0);
        CPPUNIT_ASSERT(v == 0);
        CPPUNIT_ASSERT(num_vertices(g0) == s + 1);}

    void test_add_vertex_2 () {
	    vertices_size_type s = num_vertices(g);
		CPPUNIT_ASSERT(s == 8);
        vertex_descriptor v = add_vertex(g);
        CPPUNIT_ASSERT(v == 8);
        CPPUNIT_ASSERT(num_vertices(g) == s + 1);}
		
	void test_add_vertex_3 () {
	    graph_type g0;
		add_vertex(g0);
        vertex_descriptor v = add_vertex(g0);
        CPPUNIT_ASSERT(v == 1);
        CPPUNIT_ASSERT(num_vertices(g0) == 2);}
		

    // -------------
    // test_add_edge
    // -------------

    void test_add_edge_1 () {
		graph_type g0;
		CPPUNIT_ASSERT(num_edges(g0) == 0);
		vertex_descriptor u = add_vertex(g0);
		vertex_descriptor v = add_vertex(g0);
        std::pair<edge_descriptor, bool> p = add_edge(u, v, g0);
        CPPUNIT_ASSERT(p.second == true);
		CPPUNIT_ASSERT(num_edges(g0) == 1);}

	void test_add_edge_2 () {
        std::pair<edge_descriptor, bool> p = add_edge(vdA, vdB, g);
        CPPUNIT_ASSERT(p.first  == edAB);
        CPPUNIT_ASSERT(p.second == false);}

	void test_add_edge_3 () {
		CPPUNIT_ASSERT(num_edges(g) == 10);
        std::pair<edge_descriptor, bool> p = add_edge(vdF, vdD, g);
        CPPUNIT_ASSERT(p.second == true);
		CPPUNIT_ASSERT(num_edges(g) == 11);}

    // ----------------------
    // test_adjacent_vertices
    // ----------------------

	void test_adjacent_vertices_1 () {
		graph_type g0;
		vertex_descriptor v = add_vertex(g0);
        std::pair<adjacency_iterator, adjacency_iterator> p = adjacent_vertices(v, g0);
        adjacency_iterator b = p.first;
        adjacency_iterator e = p.second;
        CPPUNIT_ASSERT(b == e);}

    void test_adjacent_vertices_2 () {
        std::pair<adjacency_iterator, adjacency_iterator> p = adjacent_vertices(vdA, g);
        adjacency_iterator b = p.first;
        adjacency_iterator e = p.second;
        CPPUNIT_ASSERT(b != e);
        if (b != e) {
            vertex_descriptor vd = *b;
            CPPUNIT_ASSERT(vd == vdB);}
        ++b;
        if (b != e) {
            vertex_descriptor vd = *b;
            CPPUNIT_ASSERT(vd == vdC);}}

    void test_adjacent_vertices_3 () {
        std::pair<adjacency_iterator, adjacency_iterator> p = adjacent_vertices(vdG, g);
        adjacency_iterator b = p.first;
        adjacency_iterator e = p.second;
        CPPUNIT_ASSERT(b != e);
        vertex_descriptor vd = *b;
        CPPUNIT_ASSERT(vd == vdH);
        ++b;
        CPPUNIT_ASSERT(b == e);}

    // ---------
    // test_edge
    // ---------

    void test_edge_1 () {
		graph_type g0;
		vertex_descriptor u = add_vertex(g0);
		vertex_descriptor v = add_vertex(g0);
		edge_descriptor edUV = add_edge(u, v, g0).first;
		std::pair<edge_descriptor, bool> p2 = edge(u, v, g0);
		CPPUNIT_ASSERT(p2.first  == edUV);
        CPPUNIT_ASSERT(p2.second == true);}
	
	void test_edge_2 () {
        std::pair<edge_descriptor, bool> p1 = edge(vdA, vdB, g);
		std::pair<edge_descriptor, bool> p2 = edge(vdF, vdD, g);
        CPPUNIT_ASSERT(p1.first  == edAB);
        CPPUNIT_ASSERT(p1.second == true);
        CPPUNIT_ASSERT(p2.second == false);}
	
	void test_edge_3 () {
		edFD = add_edge(vdF, vdD, g).first;
		std::pair<edge_descriptor, bool> p2 = edge(vdF, vdD, g);
        CPPUNIT_ASSERT(p2.second == true);}

    // ----------
    // test_edges
    // ----------

	void test_edges_1 () {
		graph_type g0;
		std::pair<edge_iterator, edge_iterator> p = edges(g0);
		CPPUNIT_ASSERT(p.first == p.second);};

    void test_edges_2 () {
        std::pair<edge_iterator, edge_iterator> p = edges(g);
        edge_iterator                           b = p.first;
        edge_iterator                           e = p.second;
        CPPUNIT_ASSERT(b != e);
        if (b != e) {
            edge_descriptor ed = *b;
            CPPUNIT_ASSERT(ed == edAB);}
        ++b;
        if (b != e) {
            edge_descriptor ed = *b;
            CPPUNIT_ASSERT(ed == edAC);}}

	void test_edges_3 () {
		edge_descriptor e = add_edge(vdF, vdD, g).first;
		edge_iterator it = edges(g).first;
		for(int i = 0; i < 8; i++) ++it; 
		edge_descriptor ed = *it;
		CPPUNIT_ASSERT(ed == e);}

    // --------------
    // test_num_edges
    // --------------

    void test_num_edges_1 () {
		graph_type g0;
        edges_size_type es = num_edges(g0);
        CPPUNIT_ASSERT(es == 0);}
		
	void test_num_edges_2 () {
        edges_size_type es = num_edges(g);
        CPPUNIT_ASSERT(es == 10);}
		
	void test_num_edges_3 () {
		edFD = add_edge(vdF, vdD, g).first;
        edges_size_type es = num_edges(g);
        CPPUNIT_ASSERT(es == 11);}
		
    // -----------------
    // test_num_vertices
    // -----------------

    void test_num_vertices_1 () {
		graph_type g0;
        vertices_size_type vs = num_vertices(g0);
        CPPUNIT_ASSERT(vs == 0);}
	
	void test_num_vertices_2 () {
        vertices_size_type vs = num_vertices(g);
        CPPUNIT_ASSERT(vs == 8);}
		
	void test_num_vertices_3 () {
		graph_type g0;
		add_vertex(g0);
        vertices_size_type vs = num_vertices(g0);
        CPPUNIT_ASSERT(vs == 1);}

    // -----------
    // test_source
    // -----------

	void test_source_1 () {
		graph_type g0;
		vertex_descriptor u = add_vertex(g0);
		vertex_descriptor v = add_vertex(g0);
		edge_descriptor e = add_edge(u, v, g0).first;
        vertex_descriptor vd = source(e, g0);
        CPPUNIT_ASSERT(vd == u);}
		
    void test_source_2 () {
        vertex_descriptor vd = source(edAB, g);
        CPPUNIT_ASSERT(vd == vdA);}
		
	void test_source_3 () {
		edFD = add_edge(vdF, vdD, g).first;
        vertex_descriptor vd = source(edFD, g);
        CPPUNIT_ASSERT(vd == vdF);}
	

    // -----------
    // test_target
    // -----------
	
	void test_target_1 () {
		graph_type g0;
		vertex_descriptor u = add_vertex(g0);
		vertex_descriptor v = add_vertex(g0);
		edge_descriptor e = add_edge(u, v, g0).first;
        vertex_descriptor vd = target(e, g0);
        CPPUNIT_ASSERT(vd == v);}

    void test_target_2 () {
        vertex_descriptor vd = target(edAB, g);
        CPPUNIT_ASSERT(vd == vdB);}
		
	void test_target_3 () {
		edFD = add_edge(vdF, vdD, g).first;
        vertex_descriptor vd = target(edFD, g);
        CPPUNIT_ASSERT(vd == vdD);}

    // -----------
    // test_vertex
    // -----------

    void test_vertex_1 () {
		graph_type g0;
		vertex_descriptor v = add_vertex(g0);
	    vertex_descriptor vd = vertex(0, g0);
		CPPUNIT_ASSERT(num_vertices(g0) == 1);
        CPPUNIT_ASSERT(vd == v);}
	
	void test_vertex_2 () {
	    vertex_descriptor vd = vertex(0, g);
		CPPUNIT_ASSERT(num_vertices(g) == 8);
        CPPUNIT_ASSERT(vd == vdA);}
	
	void test_vertex_3 () {
	    vertex_descriptor vd = vertex(7, g);
		CPPUNIT_ASSERT(num_vertices(g) == 8);
        CPPUNIT_ASSERT(vd == vdH);}

    // -------------
    // test_vertices
    // -------------

	 void test_vertices_1 () {
		graph_type g0;
		vertex_descriptor v = add_vertex(g0);
        std::pair<vertex_iterator, vertex_iterator> p = vertices(g0);
        vertex_iterator                             b = p.first;
        vertex_iterator                             e = p.second;
        CPPUNIT_ASSERT(b != e);
        CPPUNIT_ASSERT(*b == v);
        ++b;
        CPPUNIT_ASSERT(b == e);}

    void test_vertices_2 () {
        std::pair<vertex_iterator, vertex_iterator> p = vertices(g);
        vertex_iterator                             b = p.first;
        vertex_iterator                             e = p.second;
        CPPUNIT_ASSERT(b != e);
        if (b != e) {
            vertex_descriptor vd = *b;
            CPPUNIT_ASSERT(vd == vdA);}
        ++b;
        if (b != e) {
            vertex_descriptor vd = *b;
            CPPUNIT_ASSERT(vd == vdB);}}

	void test_vertices_3 () {
        std::pair<vertex_iterator, vertex_iterator> p = vertices(g);
        vertex_iterator                             b = p.first;
        vertex_iterator                             e = p.second;
        unsigned i = 0; 
        while (b != e) {
            vertex_descriptor vd = *b;
            CPPUNIT_ASSERT(vd == i);
			++b; ++i;}}
		
	// --------
    // test_dfs
    // --------
    
	void test_dfs () {
	    std::vector< coloredVertex<graph_type> > result;
		result = dfs(g);
		
		typename std::vector< coloredVertex<graph_type> >::iterator it;
		std::cout << std::endl;
		for(it = result.begin(); it != result.end(); ++it) {
		    std::cout << (*it).vert << ": " << (*it).pre_idx; 
			std::cout << ", " << (*it).post_idx << std::endl;
		}
	}

    // --------------
    // test_has_cycle
    // --------------
	
    void test_has_cycle_1 () {
		graph_type g0;
        CPPUNIT_ASSERT(!has_cycle(g0));
		vertex_descriptor u = add_vertex(g0);
		CPPUNIT_ASSERT(!has_cycle(g0));
		vertex_descriptor v = add_vertex(g0);
		add_edge(u, v, g0);
		CPPUNIT_ASSERT(!has_cycle(g0));
		add_edge(v, u, g0);
		CPPUNIT_ASSERT(has_cycle(g0));}
	
    void test_has_cycle_2 () {
        CPPUNIT_ASSERT(!has_cycle(g));}
		
	void test_has_cycle_3 () {
		edFD = add_edge(vdF, vdD, g).first;
        CPPUNIT_ASSERT(has_cycle(g));}	

    // ---------------------
    // test_topological_sort
    // ---------------------

	void test_topological_sort_1 () {
		graph_type g0;
		vertex_descriptor u = add_vertex(g0);
		vertex_descriptor v = add_vertex(g0);
		add_edge(v, u, g0);
        std::ostringstream out;
        topological_sort(g0, std::ostream_iterator<vertex_descriptor>(out, " "));
        CPPUNIT_ASSERT(out.str() == "1 0 ");}
	
    void test_topological_sort_2 () {
        std::ostringstream out;
        topological_sort(g, std::ostream_iterator<vertex_descriptor>(out, " "));
        CPPUNIT_ASSERT(out.str() == "6 0 2 1 3 5 7 4 ");}
		
	void test_topological_sort_3 () {
		edFD = add_edge(vdF, vdD, g).first;
        std::ostringstream out;
		try {
            topological_sort(g, std::ostream_iterator<vertex_descriptor>(out, " "));
		} catch (not_a_dag& e) {
            CPPUNIT_ASSERT(std::string(e.what()) == "The graph must be a DAG.");}
	}

    // -----
    // suite
    // -----

    CPPUNIT_TEST_SUITE(TestGraph);
	//CPPUNIT_TEST(test_dfs); //pass
	CPPUNIT_TEST(test_add_vertex_1);
	CPPUNIT_TEST(test_add_vertex_2);
	CPPUNIT_TEST(test_add_vertex_3);
    CPPUNIT_TEST(test_add_edge_1);
	CPPUNIT_TEST(test_add_edge_2); 
	CPPUNIT_TEST(test_add_edge_3);   
    CPPUNIT_TEST(test_adjacent_vertices_1);
    CPPUNIT_TEST(test_adjacent_vertices_2);
    CPPUNIT_TEST(test_adjacent_vertices_3);
    CPPUNIT_TEST(test_edge_1);
	CPPUNIT_TEST(test_edge_2);
	CPPUNIT_TEST(test_edge_3); 
    CPPUNIT_TEST(test_edges_1);
	CPPUNIT_TEST(test_edges_2);
	CPPUNIT_TEST(test_edges_3); 
    CPPUNIT_TEST(test_num_edges_1);
	CPPUNIT_TEST(test_num_edges_2);
	CPPUNIT_TEST(test_num_edges_3);
    CPPUNIT_TEST(test_num_vertices_1);
	CPPUNIT_TEST(test_num_vertices_2);
	CPPUNIT_TEST(test_num_vertices_3);
    CPPUNIT_TEST(test_source_1);
	CPPUNIT_TEST(test_source_2);
	CPPUNIT_TEST(test_source_3);
    CPPUNIT_TEST(test_target_1);
	CPPUNIT_TEST(test_target_2);
	CPPUNIT_TEST(test_target_3);
	CPPUNIT_TEST(test_vertex_1);
	CPPUNIT_TEST(test_vertex_2);
	CPPUNIT_TEST(test_vertex_3);
    CPPUNIT_TEST(test_vertices_1);
	CPPUNIT_TEST(test_vertices_2);
	CPPUNIT_TEST(test_vertices_3); 
	CPPUNIT_TEST(test_has_cycle_1);
	CPPUNIT_TEST(test_has_cycle_2);
	CPPUNIT_TEST(test_has_cycle_3);
	CPPUNIT_TEST(test_topological_sort_1);
	CPPUNIT_TEST(test_topological_sort_2);
	CPPUNIT_TEST(test_topological_sort_3);	
    CPPUNIT_TEST_SUITE_END();};


// ----
// main
// ----

int main () {
    using namespace std;
    using namespace boost;

    ios_base::sync_with_stdio(false); // turn off synchronization with C I/O
    cout << "TestGraph.c++" << endl;

    CppUnit::TextTestRunner tr;
    tr.addTest(TestGraph< adjacency_list<setS, vecS, directedS> >::suite());
    tr.addTest(TestGraph<Graph>::suite());
    tr.run();

    cout << "Done." << endl;
    return 0;}
