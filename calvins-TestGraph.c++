...
struct TestGraph : CppUnit::TestFixture {
    ...
    void test_add_edge_1 () {
        ...
        std::pair<edge_descriptor, bool> e = edge(vdA, vdD, g);
        CPPUNIT_ASSERT(p.first == e.first);
        CPPUNIT_ASSERT(p.second);
        CPPUNIT_ASSERT(e.second); }

    void test_add_edge_2 () {
        std::pair<edge_descriptor, bool> p = add_edge(vdD, vdH, g);
        std::pair<edge_descriptor, bool> e = edge(vdD, vdH, g);
        CPPUNIT_ASSERT(p.first == e.first);
        CPPUNIT_ASSERT(p.second);
        CPPUNIT_ASSERT(e.second); }

    void test_add_edge_3 () {
        std::pair<edge_descriptor, bool> p = add_edge(vdF, vdB, g);
        std::pair<edge_descriptor, bool> e = edge(vdF, vdB, g);
        CPPUNIT_ASSERT(p.first == e.first);
        CPPUNIT_ASSERT(p.second);
        CPPUNIT_ASSERT(e.second); }
    ...
    void test_add_vertex () {
        vertex_descriptor nvertex = add_vertex(g);
        CPPUNIT_ASSERT(nvertex == vertex(nvertex, g) ); }
    ...
    void test_edge_3 () {
        std::pair<edge_descriptor, bool> p = edge(vdH, vdG, g);
        CPPUNIT_ASSERT(p.second == false);}
    ...
    void test_topological_sort () {
        ...
        graph_type g;
        vertex_descriptor one = add_vertex(g);
        vertex_descriptor two = add_vertex(g);
        vertex_descriptor three = add_vertex(g);
        vertex_descriptor four = add_vertex(g);
        vertex_descriptor five = add_vertex(g);
        add_edge(one, three, g).first;
        add_edge(five, three, g).first;
        add_edge(five, two, g).first;
        add_edge(three, two, g).first;
        add_edge(three, four, g).first;
        add_edge(one, five, g).first;
        topological_sort(g, std::ostream_iterator<vertex_descriptor>(out, " "));
        CPPUNIT_ASSERT(out.str() == "0 4 2 3 1 ");
    }
